...
def code_dependencies_outputs(
    ...
    for node in code:
        ...
        if isinstance(node, (ast.While, ast.For, ast.If)):
            ...
            node, (ast.Try, ast.ClassDef, ast.FunctionDef, ast.AsyncFunctionDef)
        ):
            required_names.update(
                name.id for name in core.walk(node, ast.Name))
            required_names.update(
                func.name
            ...
            for child in core.walk(
                ...
                for grandchild in ast.walk(child):
                    if (
                        isinstance(grandchild, ast.Name)
                        and grandchild.id in comp_created
                    ):
                        generator_internal_names.add(grandchild)

            if isinstance(node, ast.AugAssign):
                node_needed.update(
                    n.id for n in parsing.assignment_targets(node))
        ...
        temp_created, temp_maybe_created, temp_needed = code_dependencies_outputs(
            temp_children
        )
        ...
        for nodes in children:
            c_created, c_maybe_created, c_needed = code_dependencies_outputs(
                nodes)
...
def trace_origin(
    ...
    if __all__:
        all_template = ast.Assign(
            ...
            value=ast.List(elts={ast.Constant(value=str)}),
        )
        ...
        if all_nodes:
            for node in all_nodes:
                all_filter.update(
                    constant.value for constant in node.value.elts)

            for node in core.walk(root, all_extend_template):
                all_filter.update(
                    constant.value for constant in node.args[0].elts)
        ...
        if isinstance(node, (ast.Import, ast.ImportFrom)):
            for alias in node.names:
                ...
                if node.module in constants.PYTHON_311_STDLIB:
                    ...
                    if name in exports:
                        return _TraceResult(
                            core.get_code(node, source), node.lineno, node
                        )
...
def fix_reimported_names(source: str) -> str:
    ...
    import_insert_lineno = min(
        (node.lineno for node in core.walk(root, (ast.ImportFrom, ast.Import))),
        default=-1,
    )
                ...
                *_, module_import_node = trace_result
                if isinstance(module_import_node, ast.ImportFrom):
                    ...
                    original_name = next(
                        ...
                        if alias.asname == name
                        or (alias.asname is None and alias.name == name)
                    )
                    if asname == original_name:
                        new_alias = ast.alias(name=original_name, asname=None)
                    else:
                        new_alias = ast.alias(
                            name=original_name, asname=asname)

                    module_from_imports[module_import_node.module].add(
                        new_alias)
                    ...
                    yield None, ast.Import(
                        names=[new_alias], lineno=import_insert_lineno
                    )
                else:
